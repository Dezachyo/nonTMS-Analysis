...
# Load data -> resample -> epoching -> reref -> Highpass (0.1Hz) -> ica? -> autoreject 
...
for sub_num in sub_list:
    ...
    #init log file
    log_file_path = results_path / f"log_{sub_str}.log"
    ...
    # Create a new file handler for each subject
    ...
    #TODO Save epochs
    ep_fname = f'{sub_str}_prepro-epo.fif'
    epochs.save(results_path/ep_fname)
    
    msg = f'{len(epochs)} Epochs Saved To Disc'
    logging.info(msg)

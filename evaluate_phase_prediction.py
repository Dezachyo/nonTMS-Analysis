...
for fname,sub_num in zip(tep_files,sub_list):
    ...
    duration = 0.8  # total duration of the wavelet in seconds
    ...
    psd = epochs_tep.copy().crop(tmin =-1 ,tmax = 0.6).compute_psd(**psd_kw,**welch_kw)
    group_psd.append(psd)
...
for epochs,psd, sub_num in zip(group_data,group_psd,sub_list):
    
    
    # Create the figure and subplots
    fig = plt.figure(figsize=(15, 10))
    fig.suptitle(f'Subject {sub_num}')

    # Create a polar subplot
    ax1 = fig.add_subplot(311, polar=True)  # 1 row, 2 columns, 1st subplot (polar)

    # Create a regular Cartesian subplot
    ax2 = fig.add_subplot(312)  # 1 row, 2 columns, 2nd subplot (regular)
    ax3 = fig.add_subplot(313)
    plot_phase_amp_vectors_with_mean_and_angle_sd(epochs.metadata, 'angle_diff_deg', 'point_amp_conv' ,ax1)
    plot_compare_average_epochs(epochs.copy().crop(time_to_predict - 0.5 , time_to_predict+0.2),
                                args,
                                time_to_predict,
                                ax2)

    psd.plot(xscale = 'log',picks = args.source_ch,axes = ax3)
    ax3.set_xlim(4,60)
...
# Create the figure and subplots
fig, axes = plt.subplots(nrows=nrows, ncols=ncols, subplot_kw={'polar': True}, figsize=(15, 15))
axes = axes.flatten()  # Flatten the array of axes for easy iteration

# Iterate over each subject DataFrame and corresponding subplot axis
for sub_num, epochs, ax in zip(sub_list,group_data, axes):
    plot_phase_amp_vectors_with_mean_and_angle_sd(epochs.metadata, 'angle_diff_deg', 'point_amp_conv' ,ax)
    ax.set_title(f'Subject {sub_num}')
